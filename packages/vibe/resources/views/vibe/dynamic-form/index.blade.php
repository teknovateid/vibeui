...
@php
    ...
    $addText = $addText ?? __('vibe/dynamic-form.add_row');
    $confirmDeleteMessage = $confirmDeleteMessage ?? __('vibe/dynamic-form.delete_row') . '?';
...
<div 
    id="{{ $dfId }}"
    {{ $attributes->twMerge(['class' => trim("w-full space-y-3.5 {$wrapperClass}")]) }}
    x-data="typeof window.vibeDynamicForm === 'function' ? window.vibeDynamicForm({
        id: '{{ $dfId }}',
        name: '{{ $name }}',
...
>
    ...
    @if ($label || $description)
        <div class="flex items-start justify-between gap-4 pb-1">
            ...
            <span 
                x-show="itemCount > 0"
                class="text-[11px] font-mono font-medium text-muted-foreground bg-muted/60 px-2 py-0.5 rounded-full shrink-0 select-none"
            >
                <span x-text="itemCount"></span>
                @if ($max !== null)
                    <span class="text-muted-foreground/60">/{{ $max }}</span>
                @endif
                <span>{{ __('vibe/dynamic-form.item_prefix', ['index' => '']) }}</span>
            </span>
        </div>
    @endif

    {{-- Empty State (displayed when itemCount == 0) --}}
    <vibe:dynamic-form.empty 
        :title="$emptyTitle" 
    ...
    <div class="pt-1 flex items-center justify-between gap-3">
        <vibe:button 
            ...
            x-bind:class="{ 'opacity-50 pointer-events-none': !canAdd }"
        >
        ...
        @if ($max !== null)
            ...
            >
                {{ __('vibe/dynamic-form.max_reached', ['max' => $max]) }}
            </p>
        @endif
    </div>
</div>
