...
(function(root, factory) {
    ...
    var VibeQrCode = {
        ...
        generateMatrix: function(text, levelName) {
            var level = this.Level[String(levelName || 'M').toUpperCase()] !== undefined ? this.Level[String(levelName || 'M').toUpperCase()] : QRErrorCorrectLevel.M;
        ...
        renderSvg: function(text, options) {
            ...
            var qr = this.generateMatrix(text, level);
            var count = qr.getModuleCount();
            var totalSize = count + (margin * 2);
            ...
            var bgRect = background && background !== 'transparent'
                ? '<rect width="' + totalSize + '" height="' + totalSize + '" fill="' + background + '"/>'
                : '';

            var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + totalSize + ' ' + totalSize + '" width="' + size + '" height="' + size + '" shape-rendering="crispEdges" style="display: block; max-width: 100%; height: auto;">'
                + bgRect
        ...
        downloadSvg: function(svgHtml, filename) {
            filename = filename || 'qrcode.svg';
        ...
        downloadPng: function(svgHtml, size, filename) {
