    ...
    <div :class="(typeof isInitialized !== 'undefined' && !isInitialized) ? '' : 'transition-[max-width,opacity,margin] duration-300 ease-in-out'" class="flex flex-1 min-w-0 w-full items-center justify-between overflow-hidden {{ $collapsed ? 'max-w-0 opacity-0 ml-0 hidden' : 'max-w-[100vw] opacity-100 ml-3' }} group-data-[collapsed=true]/nav-item:hidden group-data-[state=minified]/sheet:hidden">
        ...
        <div class="flex items-center gap-2 shrink-0 ml-auto">
            ...
            <div x-show="(! (typeof isGroupChild !== 'undefined' && isGroupChild)) && ((typeof pinnable !== 'undefined' && pinnable) || {{ $pinnable ? 'true' : 'false' }})" @click.prevent="if(typeof togglePin !== 'undefined') togglePin('{{ $itemId }}')" class="p-1 rounded hover:bg-vibe-200 dark:hover:bg-vibe-800 transition-all duration-150" :class="typeof isPinned !== 'undefined' && isPinned('{{ $itemId }}') ?
                ...
                {{-- Use :class hidden instead of x-show to avoid Alpine re-triggering display
                     toggling during init — which causes FOUC even when inline styles are pre-set --}}
                <!-- Pinned Icon -->
                <svg :class="(typeof isPinned !== 'undefined' && isPinned('{{ $itemId }}')) ? '' : 'hidden'" class="size-3 hidden" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16">
                ...
                <!-- Unpinned Icon -->
                <svg :class="(typeof isPinned !== 'undefined' && isPinned('{{ $itemId }}')) ? 'hidden' : ''" class="size-3" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16">
                    <path d="M0 0h16v16H0z" fill="none" />
