            ...
            <script>
                (function() {
                    try {
                        ...
                        if (stored) {
                            let data = JSON.parse(stored);
                            let navEl = document.getElementById('{{ $gridId }}').closest('nav');
                            let navId = navEl ? (navEl.dataset.navId || navEl.id) : 'sidebar-menu';
                            let item = Array.isArray(data) ? data.find(i => i.id === navId) : null;
                            if (item && item.groups && item.groups['{{ $groupId }}'] !== undefined && !{{ $active ? 'true' : 'false' }}) {
                                let isOpen = Boolean(item.groups['{{ $groupId }}']);
                                document.getElementById('{{ $gridId }}').style.gridTemplateRows = isOpen ? '1fr' : '0fr';
                                let chevron = document.getElementById('{{ $chevronId }}');
                                if (chevron) chevron.style.transform = isOpen ? 'none' : 'rotate(-90deg)';
                            }
                        }
                    } catch (e) {}
                })();
