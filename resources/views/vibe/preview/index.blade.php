                ...
                getStored: function(id) {
                    try {
                        ...
                        if (stored) {
                            ...
                            return Array.isArray(data) ? data.find(function(i) {
                                return i.id === id;
                            }) : data[id];
                        }
                    } catch (e) {}
...
<div id="{{ $previewId }}" data-toc-ignore x-data="{
    id: '{{ $previewId }}',
    ...
    <div class="relative flex items-center justify-between min-h-10.5 px-3.5 py-2 bg-muted/40 dark:bg-muted/20 border-b border-border text-xs">
        ...
        <div class="relative z-10 flex items-center gap-2 shrink-0">
            <vibe:button.group>
                ...
                <vibe:button size="xs" variant="plain" :x-data="false" data-vibe-btn-tab="code" type="button" @click="tab = 'code'; $nextTick(() => {
        ...
        <div class="relative z-10 flex items-center justify-end gap-1.5 shrink-0">
            ...
            @if ($viewports)
                <vibe:button.group id="{{ $viewportGroupId }}" x-show="tab === 'preview'" class="hidden sm:inline-flex" style="{{ $tab === 'code' ? 'display: none;' : '' }}">
                    {{-- Desktop 100% --}}
                    <button data-vibe-btn-viewport="100%" type="button" @click="viewport = '100%'" :class="{
                    ...
                    {{-- Tablet 768px --}}
                    <button data-vibe-btn-viewport="768px" type="button" @click="viewport = '768px'" :class="{
                    ...
                    {{-- Mobile 375px --}}
                    <button data-vibe-btn-viewport="375px" type="button" @click="viewport = '375px'" :class="{
            ...
            @if ($themeSwitch)
                <vibe:button.group id="{{ $themeGroupId }}" x-show="tab === 'preview'" style="{{ $tab === 'code' ? 'display: none;' : '' }}">
                    {{-- Auto / System Mode --}}
                    <button data-vibe-btn-theme="auto" type="button" @click="canvasTheme = 'auto'" :class="{
                    ...
                    {{-- Force Light Mode --}}
                    <button data-vibe-btn-theme="light" type="button" @click="canvasTheme = 'light'" :class="{
                    ...
                    {{-- Force Dark Mode --}}
                    <button data-vibe-btn-theme="dark" type="button" @click="canvasTheme = 'dark'" :class="{
    ...
    <div id="{{ $canvasId }}" x-show="tab === 'preview'" data-canvas-theme="{{ $canvasTheme }}" :data-canvas-theme="canvasTheme" class="relative w-full rounded-b-2xl transition-colors duration-300 {{ $canvasTheme === 'dark' ? 'dark' : ($canvasTheme === 'light' ? 'light' : '') }}" :class="{
    ...
    {{-- Code Viewer Area --}}
    <div id="{{ $codeId }}" x-show="tab === 'code'" class="rounded-b-2xl overflow-hidden" style="{{ $tab === 'code' ? 'display: block;' : 'display: none;' }}">
                ...
                if (item) {
                    ...
                    if (item.tab) {
                        ...
                        if (item.tab === 'code') {
                            ...
                            if (btnPreview) {
                                activeTabClasses.forEach(function(c) {
                                    btnPreview.classList.remove(c);
                                });
                                inactiveTabClasses.forEach(function(c) {
                                    btnPreview.classList.add(c);
                                });
                            }
                            if (btnCode) {
                                inactiveTabClasses.forEach(function(c) {
                                    btnCode.classList.remove(c);
                                });
                                activeTabClasses.forEach(function(c) {
                                    btnCode.classList.add(c);
                                });
                            }
                    ...
                    if (item.viewport) {
                        ...
                        vpButtons.forEach(function(btn) {
                            if (btn.getAttribute('data-vibe-btn-viewport') === item.viewport) {
                                inactiveVpClasses.forEach(function(c) {
                                    btn.classList.remove(c);
                                });
                                activeVpClasses.forEach(function(c) {
                                    btn.classList.add(c);
                                });
                            } else {
                                activeVpClasses.forEach(function(c) {
