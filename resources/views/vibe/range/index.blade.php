...
<div {{ $attributes->only('class')->twMerge(['class' => trim("{$wrapperBaseClasses} {$wrapperClass}")]) }} x-data="{
    ...
    <div class="relative flex items-center w-full select-none py-1">
        ...
        {{-- Native Range Input --}}
        <input type="range" x-ref="range" id="{{ $id }}" @if (!$isStrict && $name) name="{{ $name }}" @endif min="{{ $computedMin }}" max="{{ $computedMax }}" step="{{ $computedStep }}" value="{{ $initialSliderVal }}" @input="updateProgress($el)" @change="updateProgress($el)" @wheel.prevent="handleWheel($event)" @if ($isDisabled) disabled @endif @if ($isReadonly) readonly @mousedown.prevent @keydown.prevent @endif @if ($hasError) aria-invalid="true" @endif @if ($describedByString) aria-describedby="{{ $describedByString }}" @endif style="--range-track-height: {{ $trackHeight }}; --range-thumb-size: {{ $thumbSize }}; --range-active-color: {{ $activeColor }}; --range-track-color: {{ $trackColor }}; --progress: {{ $initialProgress }}%;" {{ ($isStrict ? $attributes->whereDoesntStartWith('wire:model') : $attributes)->except(['class', 'disabled', 'readonly'])->twMerge(['class' => $sliderClasses]) }} />
    </div>
