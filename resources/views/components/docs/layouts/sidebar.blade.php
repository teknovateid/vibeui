...
<x-docs.layouts.base :title="$title">
    <div class="flex min-h-screen overflow-hidden">
        <vibe:sheet id="sidebar-menu" position="left" layout="relative" :resizable="true" behavior="minify" minSize="200" minifiedSize="80" :persist="true">
            <vibe:sheet.header class="flex items-center justify-between minified:justify-center p-3 border-none">
                <h1 class="text-2xl font-bold block minified:hidden truncate transition-opacity duration-300">{{ config('app.name') }}</h1>
                <div class="hidden minified:flex items-center justify-center size-9 rounded-lg bg-vibe-200 dark:bg-vibe-800 font-bold text-xl transition-all duration-300">
        ...
        <div class="flex flex-col flex-1 w-full">
            <vibe:header class="bg-transparent! border-none!">
                <vibe:header.heading class="gap-2 flex items-center">
                    <vibe:button variant="ghost" class="p-2 text-vibe-600 dark:text-vibe-400 hover:text-vibe-950 dark:hover:text-vibe-50 transition-colors" @click.stop="$dispatch('toggle-sheet', 'sidebar-menu')">
                        <div x-data="{
                            state: 'expanded',
                            init() {
            ...
            <main class="flex-1 p-6 overflow-y-auto scrollbar-gutter-stable">
                {{ $slot }}
            </main>
        </div>
