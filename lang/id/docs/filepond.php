...
return [
    ...
    'reorder_section' => [
        ...
        'desc' => 'Tambahkan prop <code class="px-1.5 py-0.5 rounded bg-muted text-xs font-mono text-foreground">reorder</code> untuk mengaktifkan fitur pengurutan ulang berkas secara drag & drop. Urutan visual di antarmuka akan langsung tersinkronkan ke input form saat disubmit, sehingga berkas lama yang sudah tersimpan maupun berkas yang baru diunggah akan selalu mempertahankan posisi urutannya.',
        'preview_title' => 'Galeri Reorder Berkas (Drag & Drop)',
        'label' => 'Galeri Produk (Bisa Diurutkan Ulang)',
        'description' => 'Tahan dan seret ikon pegangan atau klik tombol panah ↑↓ untuk mengatur urutan berkas.',
        'submit_btn' => 'Simpan Urutan Galeri (Test Request)',
        'concept_title' => 'Tantangan PHP & Mekanisme Pengurutan di Vibe UI',
        'concept_desc' => 'Secara bawaan, PHP memisahkan data formulir ke dalam dua superglobal berbeda: berkas lama (URL/string) dikirim via <code class="font-mono text-xs text-primary font-semibold">$_POST</code> (<code class="font-mono text-xs text-primary">$request->input()</code>), sedangkan berkas baru dikirim via <code class="font-mono text-xs text-primary font-semibold">$_FILES</code> (<code class="font-mono text-xs text-primary">$request->file()</code>). Komponen Vibe UI secara otomatis memberikan indeks numerik berurutan (<code class="font-mono text-xs text-primary">gallery[0]</code>, <code class="font-mono text-xs text-primary">gallery[1]</code>, dst.) sesuai urutan visual tampilan di UI saat disubmit, sehingga keduanya dapat digabungkan kembali secara presisi di Controller.',
        'tab_controller' => 'Controller & Eloquent',
        'tab_trait' => 'Trait HandlesOrderedUploads',
        'tab_validation' => 'Form Request Validation',
        'tab_blade' => 'Blade View',
        'tab_migration' => 'Migrasi & Model',
        'controller_card_title' => 'Implementasi Controller (Menangani Berkas Baru & Lama)',
        'controller_card_desc' => 'Contoh implementasi lengkap untuk memproses array campuran (berkas lama dan berkas baru), menyimpan berkas baru ke storage, memperbarui kolom <code class="font-mono text-xs text-primary">sort_order</code>, serta menghapus berkas fisik dan database yang dikeluarkan pengguna dari galeri.',
        'trait_card_title' => 'Trait Reusable: HandlesOrderedUploads',
        'trait_card_desc' => 'Trait pembantu yang dapat digunakan di berbagai controller untuk menggabungkan input POST dan FILES dengan mempertahankan indeks urutan numerik visual tanpa tertimpa atau hilang.',
        'validation_card_title' => 'Validasi Array Campuran pada Form Request',
        'validation_card_desc' => 'Validasi aturan untuk memastikan setiap elemen array berupa instance UploadedFile yang valid (format ekstensi & batas ukuran) atau string URL/path berkas lama.',
        'blade_card_title' => 'Penggunaan Komponen pada Form Edit Blade',
        'blade_card_desc' => 'Cara memuat daftar berkas yang sudah ada dari database dalam urutan yang benar menggunakan prop <code class="font-mono text-xs text-primary">:files</code>.',
        'migration_card_title' => 'Skema Database & Relasi Eloquent',
        'migration_card_desc' => 'Contoh skema migrasi tabel pendukung galeri dengan kolom <code class="font-mono text-xs text-primary">sort_order</code> dan konfigurasi relasi model Eloquent terkait.',
        'quick_tip_title' => 'Tips Singkat: Union Array Satu Baris (Single Field)',
        'quick_tip_desc' => 'Jika Anda hanya memproses satu field galeri sederhana tanpa struktur array bertingkat, Anda dapat menggabungkannya secara ringkas:',
        'json_tip_title' => 'Pendekatan Alternatif: Kolom JSON',
        'json_tip_desc' => 'Jika Anda tidak menggunakan tabel relasi terpisah, Anda dapat menyimpan daftar path relatif langsung ke dalam kolom bertipe JSON di tabel utama:',
    ],
