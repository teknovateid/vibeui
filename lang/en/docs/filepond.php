...
return [
    ...
    'reorder_section' => [
        ...
        'desc' => 'Add the <code class="px-1.5 py-0.5 rounded bg-muted text-xs font-mono text-foreground">reorder</code> prop to enable drag & drop file reordering. The visual order in the UI is automatically synced to form inputs on submit, ensuring both preloaded and newly uploaded files maintain their exact positions.',
        'preview_title' => 'File Gallery Reorder (Drag & Drop)',
        'label' => 'Product Gallery (Reorderable)',
        'description' => 'Hold and drag the grip icon or click arrow buttons to reorder files.',
        'submit_btn' => 'Save Gallery Order (Test Request)',
        'concept_title' => 'PHP Superglobal Challenge & Vibe UI Order Mechanism',
        'concept_desc' => 'By default, PHP separates input data into two distinct superglobals: pre-existing files (URLs/strings) are sent via <code class="font-mono text-xs text-primary font-semibold">$_POST</code> (<code class="font-mono text-xs text-primary">$request->input()</code>), while new files are sent via <code class="font-mono text-xs text-primary font-semibold">$_FILES</code> (<code class="font-mono text-xs text-primary">$request->file()</code>). Vibe UI automatically assigns sequential numeric indices (<code class="font-mono text-xs text-primary">gallery[0]</code>, <code class="font-mono text-xs text-primary">gallery[1]</code>, etc.) matching the visual layout on submit, allowing them to be cleanly reconciled in your Controller.',
        ...
        'tab_migration' => 'Migration & Model',
        'controller_card_title' => 'Controller Implementation (Handling Existing & New Files)',
        'controller_card_desc' => 'Complete example demonstrating how to process a mixed array (old and new files), store new uploads, update <code class="font-mono text-xs text-primary">sort_order</code>, and clean up deleted files.',
        'trait_card_title' => 'Reusable Trait: HandlesOrderedUploads',
        'trait_card_desc' => 'Helper trait to merge POST input and FILES while strictly preserving the visual numeric index sequence without collisions.',
        'validation_card_title' => 'Mixed Array Form Request Validation',
        'validation_card_desc' => 'Validation rules ensuring each item is either a valid UploadedFile instance (checking mime & size) or an existing string URL/path.',
        'blade_card_title' => 'Using the Component in Edit Blade Forms',
        'blade_card_desc' => 'How to supply ordered existing files from your database using the <code class="font-mono text-xs text-primary">:files</code> prop.',
        'migration_card_title' => 'Database Schema & Eloquent Relations',
        'migration_card_desc' => 'Example migration table with a <code class="font-mono text-xs text-primary">sort_order</code> column and model relationship.',
        'quick_tip_title' => 'Quick Tip: Single-Line Array Union',
        'quick_tip_desc' => 'If you only need to handle a single gallery field without deeply nested inputs, you can merge them in one line:',
        'json_tip_title' => 'Alternative Approach: JSON Column',
        'json_tip_desc' => 'If you do not use a separate relation table, you can store the array of ordered file paths directly into a JSON column on the parent table:',
    ],
